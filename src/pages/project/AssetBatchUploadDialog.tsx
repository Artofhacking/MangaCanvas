...
import {
  ...
  fileNameToAssetName,
  type AssetBatchUploadCreateData,
  type AssetBatchUploadKind,
} from "./assetBatchUpload"

const imageMimeTypes = ["image/jpeg", "image/png", "image/jpg", "image/webp", "image/gif"]

const kindCopy: Record<
  AssetBatchUploadKind,
  {
    noun: string
    title: string
...
> = {
  character: {
    ...
    uploading: "正在上传并创建角色…",
    imageHint: "支持 JPG / PNG / WEBP / GIF，多张图片将批量创建为角色",
    zipHint: "支持 zip 格式，包内图片将批量创建为角色",
    fallbackName: "未命名角色",
  },
  scene: {
    noun: "场景",
    ...
    directory: "scenes",
    uploading: "正在上传并创建场景…",
    imageHint: "支持 JPG / PNG / WEBP / GIF，多张图片将批量创建为场景",
    zipHint: "支持 zip 格式，包内图片将批量创建为场景",
    fallbackName: "未命名场景",
  },
  object: {
    noun: "物品",
    ...
    directory: "objects",
    uploading: "正在上传并创建物品…",
    imageHint: "支持 JPG / PNG / WEBP / GIF，多张图片将批量创建为物品",
    zipHint: "支持 zip 格式，包内图片将批量创建为物品",
    fallbackName: "未命名物品",
  },
}

interface AssetBatchUploadDialogProps {
  open: boolean
  onOpenChange: (open: boolean) => void
  projectId: number
  kind: AssetBatchUploadKind
  onCreate: (data: AssetBatchUploadCreateData) => Promise<unknown>
}

export default function AssetBatchUploadDialog({
  open,
  onOpenChange,
  projectId,
  kind,
  onCreate,
}: AssetBatchUploadDialogProps) {
  ...
  const reportResult = (success: number, failed: number) => {
    if (success === 0) {
      notify.warning(failed > 0 ? `没有成功创建${copy.noun}` : `未检测到可上传的${copy.noun}图片`)
      return
    }
    notify.success(
      failed > 0
        ? `成功上传 ${success} 个${copy.noun}到素材库，${failed} 个失败`
        : `成功上传 ${success} 个${copy.noun}到素材库`,
    )
    onOpenChange(false)
  }

  const createAssetFromFile = async (file: File) => {
    const url = await uploadApi.uploadSingleFile(file, copy.directory, projectId)
    const name = fileNameToAssetName(file.name, copy.fallbackName)
    const created = await onCreate(buildUploadedAssetPayload(kind, name, url))
    if (!created) {
      throw new Error(`创建${copy.noun}失败`)
    }
  }
  ...
  const processImageFiles = async (files: File[]) => {
    ...
    for (let i = 0; i < imageFiles.length; i++) {
      ...
      try {
        await createAssetFromFile(imageFiles[i])
        success++
      } catch (err) {
        failed++
  ...
  const processZipFile = async (file: File) => {
    ...
    try {
      ...
      for (let i = 0; i < imageFiles.length; i++) {
        ...
        try {
          await createAssetFromFile(imageFile)
          success++
        } catch (err) {
          failed++
              ...
              >
                <div className="flex flex-col items-center justify-center gap-3 text-center">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-[hsl(var(--secondary))]">{copy.imageHint}</p>
                  </div>
                </div>
              </div>
              ...
              >
                <div className="flex flex-col items-center justify-center gap-3 text-center">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-[hsl(var(--secondary))]">{copy.zipHint}</p>
                  </div>
                </div>
              </div>
            </>
